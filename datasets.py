...
import numpy as np
import pickle
from custom_dataset import CustomDataset, PixmoPointsDataset
...
def get_pixmo_data(pixmo_dir,split):
    with open(f'{pixmo_dir}/{split}_data.pkl', 'rb') as f:
        data = pickle.load(f)
    return PixmoPointsDataset(data, split)