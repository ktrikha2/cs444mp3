...
class PixMoModel(nn.Module):
    ...
    def __init__(self, num_classes, feature_dim=512, num_heads=2, num_layers=2,
        ...
        nn.init.trunc_normal_(self.pos_embedding, std=0.1) #initialize wihtouyt 0s
    ...
    def forward(self, x, crop_image, points=None):
        ...
        #add pos embedding
        tokens = tokens + self.pos_embedding[:, :tokens.size(1), :].to(tokens.device) #will add back #added the scale factor 0.05
